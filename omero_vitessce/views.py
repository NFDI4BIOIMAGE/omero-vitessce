...
def generate_config(request, obj_type, obj_id, conn=None, **kwargs):
    ...
    Then open the viewer with the autogenerated config.
    """
    obj_id = int(obj_id)
    vitessce_config = create_config(request.POST, obj_type, obj_id, conn)

    # Save the config file as an attachement
    if request.POST.get("config_save"):
        ...
        viewer_url = build_attachment_viewer_url(config_id)

    # Encode the config file in the url
    else:
        viewer_url = build_json_viewer_url(vitessce_config)
...
def vitessce_json_config(request, url_config, **kwargs):
    """Decodes the URL encoded json vitessce config and
    generates a json response with it.
    """
    url_config = unquote_plus(url_config)
    config_dict = json.loads(url_config)
